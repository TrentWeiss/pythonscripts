...
parser = argparse.ArgumentParser("Automatically copy dll dependencies from vcpkg. Recursively searches a build tree for dll files\
                                  that vcpkg copied in as part of the build.  This script then copies that file to a specified install\
                                  directory IF AND ONLY IF that same dll file is also found in a specified vcpkg bin folder.")
...
parser.add_argument("--vcpkgbin", type=str, default=os.getenv("VCPKG_BIN_DIR"), required=False, help="bin directory of vcpkg installation")
